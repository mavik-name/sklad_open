...
Manuscripts, DOCX/TXT, warehouse copies, release ZIPs, chat/model memory, summaries, checkpoints and recovery English files may not generate translation prose.
...
The Ukrainian book is completed, but the project-specific English STOP remains in force until explicit author permission to translate. Intentionally deleted Social English files must not be restored unless that decision changes.
...
- Chapter 2 `Глина`: reader lines 1232–2040; recovery gap 1966–2040 translated directly from site.
- Chapter 3 `Збори фонду`: reader lines 2041–2878.

Chapter 3 parity removed recovery duplicates `I’m a mother...` and `Nothing mystical`.

### CHAPTER 4 — FULL RECOVERY TRIAGE COMPLETE, NOT SITE-VERIFIED
- quarantine parts 01–38 mechanically inspected;
- all 38 parts covered by a separate non-canonical manuscript comparison heuristic;
- 09→10 confirmed chunking duplicate relative to manuscript: repeated `Serhii was silent.`;
- 21→22 confirmed chunking duplicate relative to manuscript: repeated start of `Second. Don’t give...`;
...
- manuscript continues immediately after part 26, so stored recovery is definitely truncated inside Chapter 5 relative to the manuscript.
...
Therefore **there is no hidden Part 27 to recover from Git history**. The translation stopped at Part 26 immediately before the hard-source stop.
...
File Library exposes the manuscript for recovery comparison, but the manuscript remains forbidden as translation source. No usable canonical R212 reader snapshot has been retrieved from the warehouse or release directories.
...
## RECOVERY ORDER
`main` → safety branch → Git history/tree/blob → translation recovery workspaces → File Library.
