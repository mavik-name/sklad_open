# PHYSICALLY RECOVERED ENGLISH BOOKS
...
The exact English book directories below were restored from immutable archival Git tree SHAs and physically exist under:
`mavik-sklad/translations/en/books/`

Complete recovered titles:
...
On 2026-08-18 all nine directory tree SHAs were confirmed identical between `main` and the safety branch. This proves physical mirror integrity only. It does NOT prove parity with the canonical Ukrainian publication on mavik.name and does not make the recovered English automatically publication-ready.

Partial recovered work:
- `Three Novels Nobody Reads` — Sections 1–8 under `mavik-sklad/translations/en/partial/`; the partial tree SHA is identical across both branches.
...
Recovery tree SHAs and history notes:
`mavik-sklad/translations/en/RECOVERY-MAP.md`

Never declare non-excluded work lost after checking only one folder, and never confuse recovery integrity with source parity.
