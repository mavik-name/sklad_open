...
Do NOT use File Library manuscripts, DOCX/TXT, warehouse manuscripts, historical release copies, recovery English files, summaries, checkpoints or chat/model memory to generate translation prose.
...
4. translation recovery branches/workspaces as read-only recovery layers;
5. File Library as recovery evidence only.

Explicit author deletion/exclusion overrides preservation. In particular, English `Соціальна` material intentionally deleted under the STOP decision must not be restored unless the author explicitly reverses that decision.
