...
- [DONE] 2026-09-07 — Implemented and syntax-validated `_app/Core/Modules/ModuleCatalog.php` in the Full Build working tree. It auto-discovers installed modules, maintains enabled state, protected/system modules, dependencies, Boss menu visibility and ordering while retaining compatibility with boolean `modules.json`.
- [DONE] 2026-09-07 — Wired ModuleCatalog into bootstrap and LegacyModuleManager so runtime and Boss share one enabled-state source instead of independent manifest interpretation.
- [DONE] 2026-09-07 — Added/normalized `module.json` metadata for Analytics, Author, Releases, Search, Support, System and Warehouse; hardened Boss/System as protected/non-replaceable system modules.
- [DONE] 2026-09-07 — Migrated Boss ModuleManager to ModuleCatalog. Boss actions now use the canonical catalog for connect/disconnect and menu visibility; destructive delete is blocked for protected modules. Module ordering state is implemented and the simplified module screen is in place. PHP syntax validation passed for Boss ModuleManager, Boss Module and Boss view.
- [DONE] 2026-09-07 — Implemented and syntax-validated `_app/Modules/Music/AudioLibrary.php`. Physical audio files under `/assets/music` are now the authoritative inventory; supported audio extensions are scanned recursively and `music.json` is used as metadata/settings overlay.
- [DONE] 2026-09-07 — Wired Music legacy module to expose `player.library` service and render scanned tracks/playlists rather than relying only on manually listed JSON tracks.
- [DONE] 2026-09-07 — Replaced Reader-owned album filtering with Player/Music contract usage. Reader now requests tracks from AudioLibrary; if no book-specific playlist is assigned, it receives the complete visible library. Book playlist mapping and multiple allowed playlists are supported in the service.

## Current action
- Finish Player Manager in Boss: make scanned/unmanaged files visible to admin, allow metadata/hide/delete/upload, playlist management, and book-to-playlist assignment UI. Then implement Blog Publisher sandbox.
