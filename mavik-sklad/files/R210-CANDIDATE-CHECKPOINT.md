# R210 — CANDIDATE CHECKPOINT · MAVIK.NAME
...
## Що перевірити на живому сайті

1. `Boss → Сторінки`: список сторінок має бути компактним, без обрізання назв.
2. `Boss → Пошта → Перевірити IMAP і SMTP`: при відсутньому PHP IMAP статус має показати `вбудований TLS IMAP` і IMAP має підключитися до Cityhost.
3. Відкрити вхідні, один лист і вкладення.
4. Повторно глянути desktop `Boss → Анонси` — попередня правка вирівнювання походить із R209.

До прямого `ОК` від автора R210 **не є канонічним**. Єдиний канонічний CORE залишається R207.
