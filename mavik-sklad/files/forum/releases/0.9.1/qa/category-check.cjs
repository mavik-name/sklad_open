...
(async()=>{
 ...
 for(const width of [1889,1448,1024,780,592,390,320]){
  ...
  const measure=()=>page.evaluate(()=>{
   ...
   return {overflow:document.documentElement.scrollWidth>innerWidth,readable:rows.every(r=>r.querySelector('.category-topic-copy').getBoundingClientRect().width>r.getBoundingClientRect().width*.45),separated:rows.every(r=>{const t=r.querySelector('strong').getBoundingClientRect(),a=r.querySelector('.category-topic-copy>span').getBoundingClientRect(),m=r.querySelector('.category-topic-meta').getBoundingClientRect(),parts=[...r.querySelector('.category-topic-meta').children].map(e=>e.getBoundingClientRect());return a.top>=t.bottom&&((m.left>=t.right)||(m.top>=a.bottom))&&(parts[1].left>=parts[0].right||parts[1].top>=parts[0].bottom)})};
  });
