...
<section class="forum-main-column">
  ...
  <?php foreach($latest as $t): ?>
    <a class="topic-row" href="/topic.php?slug=<?=urlencode($t['slug'])?>">
      ...
      <div class="topic-last"><?=avatar($t['last_avatar'],$t['last_author'],'',(int)$t['last_user_id'])?><div><strong><?=e($t['last_author'])?></strong><span><?=e(format_forum_date($t['last_activity_at']))?></span></div></div>
      <div class="arrow">›</div>
    </a>
  <?php endforeach; ?>
  </div>
...
<aside>
  <div class="panel welcome-panel"><?php if(is_logged_in()): $visitor=authenticated_user();?><a class="welcome-profile" href="/profile.php"><?=avatar($visitor['avatar_path'],$visitor['display_name'],'',(int)$visitor['id'])?><strong><?=e($visitor['display_name'])?></strong></a><?php endif;?><h3>Ласкаво просимо!</h3><p>Приєднуйтесь до спільноти, щоб брати участь в обговореннях, ділитися думками та знаходити однодумців.</p><?php if(is_admin()): ?><a class="btn primary block" href="/profile.php">Профіль</a><?php elseif(is_logged_in()): ?><a class="btn primary block" href="/profile.php">Профіль</a><?php else: ?><a class="btn primary block" href="/auth.php?mode=register">Зареєструватися</a><p class="welcome-login">Вже маєте акаунт? <a href="/auth.php">Увійти</a></p><?php endif; ?></div>
  <div class="panel"><h3><span class="heading-icon"><?=category_icon('vilne')?></span>Останні обговорення</h3><div class="aside-discussions"><?php foreach(array_slice($latest,0,5) as $t):?><a href="/topic.php?slug=<?=urlencode($t['slug'])?>"><?=avatar($t['last_avatar'],$t['last_author'],'',(int)$t['last_user_id'])?><span><strong><?=e($t['title'])?></strong><small><?=e($t['last_author'])?> · <?=e(format_forum_date($t['last_activity_at']))?></small></span></a><?php endforeach;?></div></div>
...
<?php require_once __DIR__.'/app/ads.php';forum_ad('home');?></aside>
</div>
<?php forum_footer(); ?>
