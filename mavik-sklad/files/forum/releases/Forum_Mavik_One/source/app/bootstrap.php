...
function login_rate_limit(): void {
 $pdo=db();$pdo->exec('CREATE TABLE IF NOT EXISTS login_attempts (address TEXT PRIMARY KEY, started INTEGER NOT NULL, attempts INTEGER NOT NULL)');
 $key=hash('sha256',(string)($_SERVER['REMOTE_ADDR']??'unknown'));$now=time();
 $pdo->prepare('DELETE FROM login_attempts WHERE started<?')->execute([$now-900]);
 $pdo->prepare('INSERT INTO login_attempts(address,started,attempts) VALUES(?,?,1) ON CONFLICT(address) DO UPDATE SET attempts=attempts+1')->execute([$key,$now]);
 $st=$pdo->prepare('SELECT attempts FROM login_attempts WHERE address=?');$st->execute([$key]);
 if((int)$st->fetchColumn()>20){http_response_code(429);header('Retry-After: 900');exit('Забагато спроб входу. Спробуйте через 15 хвилин.');}
}
...
function forum_header(string $title): void {
    ...
    $seoPublic=in_array($seoPath,['/','/index.php','/category.php','/topic.php','/topics.php','/rules.php'],true);
    $seoQuery=[];if(in_array($seoPath,['/category.php','/topic.php'],true))$seoQuery['slug']=(string)($_GET['slug']??'');
    if($seoPath==='/topics.php' && (int)($_GET['page']??1)>1)$seoQuery['page']=(int)$_GET['page'];
    $canonical='https://forum.mavik.name'.($seoPath==='/index.php'?'/':$seoPath).($seoQuery?'?'.http_build_query($seoQuery):'');
    if(!$seoPublic||$maintenance)header('X-Robots-Tag: noindex, nofollow');
